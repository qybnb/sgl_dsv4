...
from sglang.srt.hardware_backend.npu.attention.dsv4_metadata import (
    build_compress_locs,
    fill_verify_positions_boundary,
    fill_verify_positions_gather,
)
...
class DeepseekV4AscendAttnBackend(
    ...
    def __init__(
        ...
        # GPU-aligned draft handling: the NEXTN/draft decoder layer is built with
        # compress_ratio=0 (deepseek_v4_nextn.COMPRESS_RATIO_NEXTN_LAYER), and a
        # ratio-0/1 layer has no Compressor/C4Indexer and never uses compress
        # state. Drop the compress ratios for the draft so the backend skips
    ...
    def update_verify_buffers_to_fill_after_draft(
        ...
        """Refresh verify compressor positions after draft graph replay.

        In graph mode the target-verify metadata is prepared before draft
        outputs are final. The fused compressor reads positions_cmp_padding_*,
        so these buffers must be rebuilt from spec_info.positions after draft
        finishes, matching the lkl_old NPU path.
        """
    ...
    def _init_dsv4_graph_buffers(self, *, max_bs: int, max_num_tokens: int) -> None:
        ...
        # build_compress_locs can emit more cols than pages/R at certain
    ...
    def init_forward_metadata_replay_cuda_graph(
        ...
        result = build_compress_locs(
            page_size=self.page_size,
            compress_ratios=self._dsv4_compress_ratios,
        ...
        # after-draft update path; the graph replay path for v1 EAGLE does not
        # call update_verify_buffers_to_fill_after_draft().
        if _verify_compress:
            verify_seq_lens_cpu = (
                seq_lens_cpu[:bs] + self.speculative_num_draft_tokens
            )
            fill_verify_positions_boundary(
            ...
            for ratio in self._dsv4_unique_compress_ratios:
                ...
                dst_loc = fm.c4_loc if ratio == 4 else fm.c128_loc
                dst_loc.zero_()
                loc = bundle_loc.to(torch.int32)
                assert loc.numel() <= dst_loc.numel(), (
        ...
        # build_compress_locs does not produce this; compute here and copy into
        # the preallocated buffer.
        swa_loc = pool.translate_loc_from_full_to_swa(out_cache_loc).to(torch.int64)
        fm.swa_loc[: swa_loc.shape[0]].copy_(swa_loc)
        fm.swa_loc[swa_loc.shape[0] :].fill_(0)
    ...
    def init_forward_metadata(self, forward_batch: "ForwardBatch") -> None:
        ...
        if (
            ...
            B = forward_batch.batch_size
            n_draft = self.speculative_num_draft_tokens or 1
    ...
    def _compute_kernel_metadata(self, forward_batch: "ForwardBatch") -> dict:
        ...
        if forward_batch.forward_mode.is_target_verify() or forward_batch.forward_mode.is_draft_extend(include_v2=True):
            max_seqlen_q = self.speculative_num_draft_tokens or 1
        else:
            max_seqlen_q = 1
    ...
    def _build_npu_compress_metadata(self, forward_batch: "ForwardBatch") -> None:
        ...
        Thin eager shim around :func:`build_compress_locs`; the pure helper
        ...
        # Fused-compressor metadata — prefill path (unchanged, excludes verify).
        if (
        ...
        # Fused-compressor metadata — verify path (mid-sequence).
        # Draft tokens are at absolute positions [committed, committed+nd);
        # start_pos = committed, compression boundaries use (abs_pos+1)%ratio.
        if _verify_compress:
            self._build_npu_compress_metadata_verify(forward_batch)

    def _build_npu_compress_metadata_verify(
        self, forward_batch: "ForwardBatch"
    ) -> None:
        """Fused-compressor metadata for target_verify (mid-sequence).

        positions are the draft tokens' ABSOLUTE positions. For each ratio,
        a compressed token is produced where (abs_pos+1)%ratio==0; its rope
        position is abs_pos-(ratio-1) (chunk start). The boundary order
        matches the densely-packed out_c{N}_loc the allocator reserved.
        start_pos = committed (= forward_batch.seq_lens).
        """
        fm = self.forward_metadata
        device = forward_batch.seq_lens.device
        ...
        # start_pos = committed (first draft position) per req.
        ...
        for ratio in self._dsv4_unique_compress_ratios:
            ...
            if bundle_loc.numel() > 0:
                assert bundle_loc.numel() <= padding_size, (
                    f"verify c{ratio}_loc overflow: "
                    f"{bundle_loc.numel()} > {padding_size}"
                )
                loc[: bundle_loc.numel()].copy_(bundle_loc.to(torch.int32))
    ...
    def _build_npu_compress_metadata_prefill(
        ...
        bundle = forward_batch.out_cache_loc_dsv4
        if bundle is None:
            raise RuntimeError("DSV4 prefill compression requires out_cache_loc_dsv4.")
        for ratio in (4, 128):
            if ratio not in ratio_lists:
                continue
            setattr(
                fm,
                f"c{ratio}_loc",
                (bundle.out_c4_loc if ratio == 4 else bundle.out_c128_loc).to(
                    torch.int32
                ),
            )
        ...
        for ratio in (4, 128):
            if ratio not in self._dsv4_compress_ratios:
                continue
            spt = fm.c4_state_page_table if ratio == 4 else fm.c128_state_page_table
            for idx in range(bs):
    ...
    def forward_compress(  # type: ignore[override]
        ...
        # Keep a compressor-specific copy to mirror lkl_old and avoid coupling
        # future attention-layout changes to the fused compressor input.
        ...
        # tensor may be shorter than the padded output; trim to len(loc). Other
        # modes must provide locs aligned to the compressor output; otherwise we
        # are hiding metadata bugs by dropping KV/state writes.
    ...
    def _compressor_epilog_npu(
        ...
        if loc is not None:
            ...
            if forward_batch.forward_mode.is_target_verify():
                valid = loc != 0
                if forward_batch.attn_backend.graph_mode:
